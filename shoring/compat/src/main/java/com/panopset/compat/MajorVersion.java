...
public enum MajorVersion {
  ...
  MajorVersion(int major, String strRep, String whenReleased) {
    this.major = major;
    this.strRep = strRep;
  ...
  public final String whenReleased;

  public static MajorVersion findFromHexString(String hexRep) throws MajorVersionException {
    if (hexRep == null) {
      throw new NullPointerException();
    }
    if (hexRep.length() != 2) {
      throw new MajorVersionException(String.join(":", "Invalid hex rep", hexRep));
    }
    int val = Integer.parseInt(hexRep, 16);
    for (MajorVersion jv : MajorVersion.values()) {
      if (val == jv.major) {
        return jv;
      }
    }
    throw new MajorVersionException("Undefined hex rep " + hexRep);
  }
