...
public class Fileop {
	...
	public static void deleteLines(File file, final String searchCriteria) {
		...
		new FileProcessor(file, lf).exec();
	}
	...
	public static void mkdirs(final File path) {
		if (!path.mkdirs()) {
			Logop.error(String.format("%s %s", Nls.xlate("Unable to create path to"), getCanonicalPath(path)));
		}
	}

	public static void copyInputStreamToFile(final InputStream inp, String fileName) throws IOException {
	...
	public static void copyFile(File ff, File tf) {
		try {
