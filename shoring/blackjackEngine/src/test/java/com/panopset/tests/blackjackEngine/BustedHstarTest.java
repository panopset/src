...
public class BustedHstarTest extends AbstractBlackjackTest {
  ...
  void testSingleDeck() {
    ...
    verifyRecommendedActions(bge, new String[] {CMD_DEAL, CMD_DOUBLE}, bustedHstarSingle());
    performDeviantActions(bge, new String[] {CMD_DEAL, CMD_HIT, CMD_HIT}, bustedHstarSingle());
  ...
  void test() {
    ...
    verifyRecommendedActions(bge, new String[] {CMD_DEAL, CMD_HIT, CMD_STAND}, bustedHstarSingle());
    performDeviantActions(bge, new String[] {CMD_DEAL, CMD_HIT, CMD_HIT}, bustedHstarSingle());
    Assertions.assertEquals("Busted",
        bge.getCycle().getPlayers().get(0).getHands().get(0).getMessage());
  }
